...
Run from the same project folder that contains data/messages_m0_4.json and font5x8.bin.
"""
...
def load_messages() -> dict:
    message_file = script_dir() / "data" / "messages_m0_4.json"
...
def expression_for_title(title: str) -> str:
    ...
    if title == "WARNING":
        return "concerned"
    if title == "FAMILY":
        return random.choice(["pleased", "happy"])
    if title == "LATENIGHT":
        return random.choice(["sleepy", "unimpressed", "judging"])
    if title == "STARTUP":
        return "processing"
...
def draw_sleepy_eye(oled, x: int, y: int):
    oled.fill_rect(x, y + 6, 11, 2, 1)
    oled.pixel(x + 1, y + 5, 1)
    oled.pixel(x + 9, y + 5, 1)


def draw_happy_eye(oled, x: int, y: int):
    # Tiny smile/arc style eye for the 16px yellow band.
    for i in range(5):
        oled.pixel(x + 1 + i, y + 6 + min(i, 4 - i), 1)
        oled.pixel(x + 6 + i, y + 6 + min(i, 4 - i), 1)


def draw_thinking_eye(oled, x: int, y: int):
    draw_square_eye(oled, x, y, pupil="up")
    oled.pixel(x + 12, y + 2, 1)
    oled.pixel(x + 14, y + 4, 1)
    oled.pixel(x + 12, y + 6, 1)
...
def draw_eyes(oled, expression: str):
    ...
    elif expression == "look_left":
        draw_square_eye(oled, left_x, y, pupil="left")
        draw_square_eye(oled, right_x, y, pupil="left")
    elif expression == "look_right":
        draw_square_eye(oled, left_x, y, pupil="right")
        draw_square_eye(oled, right_x, y, pupil="right")
    elif expression == "sleepy":
        draw_sleepy_eye(oled, left_x, y)
        draw_sleepy_eye(oled, right_x, y)
    elif expression == "happy":
        draw_happy_eye(oled, left_x, y)
        draw_happy_eye(oled, right_x, y)
    elif expression == "thinking":
        draw_thinking_eye(oled, left_x, y)
        draw_thinking_eye(oled, right_x, y)
...
def animate_message(oled, title: str, message: str, expression: str, display_title: str, duration: int):
    """Keep Vex alive between message rotations with blinks, glances, and thinking ticks."""
    end_time = time.time() + max(1, duration)
    next_blink = time.time() + random.uniform(2.0, 6.0)
    next_glance = time.time() + random.uniform(4.0, 10.0)
    thinking_tick = 0
    ...
    while time.time() < end_time:
        ...
        if now >= next_blink:
            draw_vex(oled, title, message, "blink", display_title)
            time.sleep(0.12)
            draw_vex(oled, title, message, expression, display_title)
            next_blink = now + random.uniform(2.0, 7.0)

        elif now >= next_glance:
            glance = random.choice(["look_left", "look_right", "side_eye"])
            draw_vex(oled, title, message, glance, display_title)
            time.sleep(random.uniform(0.35, 0.75))
            draw_vex(oled, title, message, expression, display_title)
            next_glance = now + random.uniform(5.0, 12.0)

        elif expression in ("processing", "thinking") and random.randint(1, 20) == 1:
            thinking_tick = (thinking_tick + 1) % 4
            dots = "." * thinking_tick
            draw_vex(oled, title, f"{message} {dots}", "thinking", display_title)

        time.sleep(MICRO_ANIMATION_TICK)
...
def main():
    ...
    while True:
        ...
        animate_message(oled, title, message, expression, display_title, args.interval)
